...
def tic_tac_toe():
  ...
  def start_game():
    ...
    while begin_option != '1' and begin_option != '2':
        begin_option = input('Quem começa? ')

        if begin_option != '1' and begin_option != '2':
            print('Opção inválida! As opções válidas são 1 e 2!')
  ...
  def end_game(turn):
    ...
    while action != '1' and action != '2':
      action = input('Deseja jogar novamente? ')

      if action != '1' and action != '2':
        print('Opção inválida, as opções disponíveis são 1 ou 2!')
